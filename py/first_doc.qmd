...
```{python}
with open("/Users/lefie/301/alice/alice.txt", "r", encoding="utf-8") as f:
  alice = f.read()
```
...
bar(anne, k=10, title="Top Words in Anne of Green Gables")
```
...
# Regular scale
line(anne, title="Rank vs Frequency (Regular)")
# Log scale (shows Zipf-like pattern clearly)
line(anne, title="Rank vs Frequency (Log Scale)", logy=True)
```
...
scatter(anne, title="Word Length vs Frequency (Normal Scale)")
```
